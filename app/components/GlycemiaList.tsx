...
export default function GlycemiaList({
  registers,
  onDelete,
}: GlycemiaListProps) {
    ...
    return (
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md text-center">
        <p className="text-gray-500 dark:text-gray-400">
          No glycemia records yet. Add your first measurement!
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
      <h2 className="text-xl font-semibold mb-4">
        Glycemia Records ({registers.length})
      </h2>
      <div className="space-y-4">
        {registers.map((register) => (
          <div
            key={register.glycemiaId}
          ...
          >
            <div className="flex justify-between items-start mb-2">
              <div className="flex items-center gap-3">
                ...
                <span
                  className={`px-2 py-1 text-xs font-medium rounded-full ${getResultColor(register.result)}`}
                >
                  {register.result}
                </span>
                <span className="text-sm text-gray-600 dark:text-gray-400">
